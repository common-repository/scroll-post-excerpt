...
function spe_show() 
{
	...
	if ( ! empty($spe_data) ) 
	{
		...
		$spe = '<div style="padding-top:8px;padding-bottom:8px;">';
			$spe .= '<div style="text-align:left;vertical-align:middle;text-decoration: none;overflow: hidden; position: relative; margin-left: 1px; height: '.$spe_height1.';" id="spe_Holder">';
				$spe .= $spe_html;
			$spe .= '</div>';
		$spe .= '</div>';
		$spe .= '<script type="text/javascript">';
		$spe .= 'var spe_array	= new Array();';
		$spe .= "var spe_obj	= '';";
		$spe .= "var spe_scrollPos 	= '';";
		$spe .= "var spe_numScrolls	= '';";
		$spe .= "var spe_heightOfElm = '".$dis_num_height."';";
		$spe .= "var spe_numberOfElm = '".$spe_count."';";
		$spe .= "var spe_speed = '".$spe_speed."';";
		$spe .= "var spe_waitseconds = '".$spe_waitseconds."';";
		$spe .= "var spe_scrollOn 	= 'true';";
		$spe .= 'function spe_createscroll()'; 
		$spe .= '{';
			$spe .= $spe_x;
			$spe .= "spe_obj	= document.getElementById('spe_Holder');";
			$spe .= "spe_obj.style.height = (spe_numberOfElm * spe_heightOfElm) + 'px';";
			$spe .= 'spe_content();';
		$spe .= '}';
		$spe .= '</script>';
		$spe .= '<script type="text/javascript">';
		$spe .= 'spe_createscroll();';
		$spe .= '</script>';
...
function spe_install() 
{
	...
	add_option('spe_dis_num_height', "150");
...
function spe_shortcode( $atts ) 
{
	//[scroll-post-excerpt setting="1"]
	echo spe_show();
}

add_shortcode( 'scroll-post-excerpt', 'spe_shortcode' );
